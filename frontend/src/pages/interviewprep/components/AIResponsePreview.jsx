      ...
      <div className="text-[14px] prose prose-slate dark:prose-invert max-w-none">
        <ReactMarkdown
          ...
          components={{
            ...
            ul({ children }) {
              return (
                <ul className="list-disc pl-6 space-y-2 ,y-4">{children}</ul>
              );
            },
            ol({ children }) {
              return (
                <ol className="list-decimal pl-6 space-y-2 ,y-4">{children}</ol>
              );
            },
            li({ children }) {
...
function CodeBlock({ code, language }) {
  ...
  return (
    <div className="relative my-6 rounded overflow-hidden bg-gray-50 border border-gray-200">
      <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200">
        <div className="flex items-center space-x-2">
          <LuCode size={16} className="text-gray-500" />
          <span className="text-xs font-semibold text-gray-600 uppercase tracking-wde">
            {language || "code"}
          </span>
        </div>
        <button
          onClick={copyCode}
          className="texy-gray-500 hover:text-gray-700 focus:outline-none relative group cursor-pointer"
        ...
        >
          ...
          {copied && (
            <span className="abpsulte -top-8 right-8 bg-black text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 transition">
              Copied!
            </span>
          )}
        </button>
